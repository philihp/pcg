import { createPcg64, getOutput64, nextState64, prevState64, randomInt64, randomList64 } from '..'
...
describe('basic', () => {
  ...
  it('pCG32_XSH_RS: Single integer', () => {
    expect.assertions(2)
    const randomUint32 = randomInt64(0, 2 ** 32 - 1)
    const pcg = createPcg64({ outputFnType: OutputFnType.XSH_RS }, 42, 54)
    // Check for generator immutability and result reproducibility
    const [n, _state] = randomUint32(pcg)
  ...
  it('pCG32_XSL_RR: Single integer', () => {
    expect.assertions(2)
    const randomUint32 = randomInt64(0, 2 ** 32 - 1)
    const pcg = createPcg64({ outputFnType: OutputFnType.XSL_RR }, 42, 54)
    // Check for generator immutability and result reproducibility
    const [n, _state] = randomUint32(pcg)
  ...
  it('pCG32_RXS_M_XS: Single integer', () => {
    expect.assertions(2)
    const randomUint32 = randomInt64(0, 2 ** 32 - 1)
    const pcg = createPcg64({ outputFnType: OutputFnType.RXS_M_XS }, 42, 54)
    // Check for generator immutability and result reproducibility
    const [n, _state] = randomUint32(pcg)
  ...
  it('pCG32_XSH_RR: Multiple integers', () => {
    ...
    const randomUint32 = randomInt64(0, 2 ** 32 - 1)
    const pcg = createPcg64({}, 42, 54)
    expect(randomList64(6, randomUint32, pcg).map(([value]) => value)).toStrictEqual([
  ...
  it('pCG32_XSH_RR: Jump-ahead, jump-back', () => {
    ...
    expect(randomUint32(nextState64(pcg))[0]).toBe(0x7b47f409)
    expect(randomUint32(prevState64(nextState64(pcg)))[0]).toBe(0xa15c02b7)

    expect(randomUint32(pcg)[1]).toStrictEqual(nextState64(pcg))

    expect(prevState64(nextState64(pcg))).toStrictEqual(pcg)
    expect(nextState64(prevState64(pcg))).toStrictEqual(pcg)
  })
  ...
  it('complains when range is wrong', () => {
    expect.assertions(1)
    const pcg = createPcg64({}, 42, 1)
    expect(() => randomInt64(0, -1, pcg)).toThrow(RangeError)
  })

  it('supports the full 32-bit output range (bound === 2^32)', () => {
    expect.assertions(2)
    const pcg = createPcg64({}, 42, 54)
    const randomFullUint32 = randomInt64(0, 2 ** 32)
    ...
    expect(() => randomInt64(0, 2 ** 32 + 1, pcg)).toThrow(RangeError)
  })
  ...
  it('make sure there are no infinite loops', () => {
    ...
    for (let x = 0; x < 256; x++) {
      for (let y = 0; y < 256; y++) {
        const pcg = createPcg64({}, x, y)
        expect(getOutput64(pcg)).not.toBe(0)
      }
    }
  })
})
