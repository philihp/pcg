...
describe('output function edge cases', () => {
  ...
  test('XSH_RS with shift at its minimum (hi top bits zero)', () => {
    // shift = (hi >>> 29) + 22. hi=0 gives shift=22 (minimum). Covers the
    // narrowest-shift branch of the xor-shift-rotate.
    const state = buildState(0, 0xdeadbeef, OutputFnType.XSH_RS)
    const out = getOutput(state)
    assert.equal(typeof out, 'number')
  })

  test('XSL_RR with hi == lo (xor cancels to zero)', () => {
    const state = buildState(0x11223344, 0x11223344, OutputFnType.XSL_RR)
    const out = getOutput(state)
    assert.equal(typeof out, 'number')
  })
})
